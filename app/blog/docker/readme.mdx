...
- `docker build -f dockerfile文件路径 -t 镜像名称:版本 . ` 使用dockerfile制作镜像，默认使用当前目录的dockerfile，主要后面还跟了一个`“.”`
...
- `docker run [OPTIONS] IMAGE [COMMAND] [ARG...] ` 根据镜像运行容器
  ...
  - example：`docker run --name nginx-test -p 8080:80 -d nginx`，`docker run --name nginx-test -p 8080:80 -it nginx bash`
  - `-it`
